...
import com.ibm.portal.portlet.service.model.MappingURLTreeModelProvider;
...
import com.ibm.workplace.wcm.api.extensions.url.ContentUrlFilterInstantiationException;
import com.ibm.workplace.wcm.api.extensions.url.ContentUrlGenerationFilter;
import com.ibm.workplace.wcm.api.extensions.url.ContentUrlGenerationFilterFactory;
...
public class FriendlyUrlGenerationFilterFactory implements ContentUrlGenerationFilterFactory {
	...
	public ContentUrlGenerationFilter getFilter(final RenderRequest request, final RenderResponse response)
		...
		if (!Boolean.valueOf(request.getPreferences().getValue(DISABLE_FILTER, Boolean.FALSE.toString()))) {
			try {
				...
				result = new FriendlyUrlGenerationFilter(
						getFriendlySelectionServiceHome().getPortletFriendlySelectionService(request, response),
						getContentModelProvider().getContentModel(request, response), workspace, getIdentification(),
						getStateManageService().getPortletStateManager(request, response),
						getMappingURLTreeModelProvider().getMappingURLTreeModel(request, response),
	 ...
	 */
	private static MappingURLTreeModelProvider getMappingURLTreeModelProvider()
	 ...
	 *             if the service is not available
	 */
	private static PortletStateManagerService getStateManageService()
			throws NamingException, PortletServiceUnavailableException {
	 ...
	 *             if the service is not available
	 */
	private static PortletFriendlySelectionServiceHome getFriendlySelectionServiceHome()
			throws NamingException, PortletServiceUnavailableException {
	 ...
	 *             if the service is not available
	 */
	private static ContentModelProvider getContentModelProvider()
			throws NamingException, PortletServiceUnavailableException {
